// Only invoke DFS once per "chain" of empty spots.
function scoreBoardIds(data, emptyIds, owner) {
...
function calcScoreIds(data) {
  ...
  let [empty, stone] = scoreBoardIds(data, emptyIds, owner);
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      let id = emptyIds[y][x];
...
function scoreIds7x7(input) {
  let data = parse(input);
  let score = calcScoreIds(data);
  return [
